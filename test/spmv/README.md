...
- `op`：矩阵的转置选项（`aclsparseOperation_t`）
- `alpha`：标量 alpha
- `mat`：稀疏矩阵描述符（`aclsparseConstSpMatDescr_t`，只读输入）
- `x`：输入向量描述符（`aclsparseConstDnVecDescr_t`，只读输入）
- `beta`：标量 beta
- `y`：输出向量描述符（`aclsparseDnVecDescr_t`）
- `computeType`：计算数据类型（`aclDataType`）
- `alg`：SPMV 算法选择（`aclsparseSpMVAlg_t`）
- `buffer`：工作空间（DEVICE）
