...
> SPMM 当前版本仅在 Ascend 950PR 平台交付，与 `src/spmm/` 的 SOC 网关保持一致。非 `ascend950*` 平台编译时会跳过 `spmm_test`。
...
│   ├── README.md           // 说明文档
│   └── spmm_test.cpp       // 算子调用样例
```

源码实现位于 `src/spmm/arch35/`：
