    ...
    void *buffer)
{
    ...
    /* β≠0 路径：数值 kernel 前将 matC values 清零。
     * 初版按 matC 初始为空、输出结构 = α·A·B 实现，β·C_in = 0，
     * 需将 valuesC 清零避免 kernel 读取未初始化数据。
     * cuSPARSE 完整语义（β≠0 且 C_in 带内容时叠加 β·C_in）为后续扩展，
     * 需用户在 Preprocess 后手动填充 C_in 并通过 API 标记 cInDataValid。
     */
    ...
    void *buffer2)
{
    ...
    /* 阶段 2：执行 — 数值 kernel 填充 C 值。
     * β≠0 + C_in 扩展：
     *   cInDataValid == 1 → 用户已填充 matC->values 为 C_in 数据（匹配符号结构），
     *     不 memset，kernel 读取 C_in。
     *   cInDataValid == 0 → C_in 为空，memset valuesC 为 0（β·C_in=0）。
     */
    float betaVal = ScalarReadF32(beta);
    if (betaVal != 0.0f && desc->cInDataValid == 1 && matCInner->values != nullptr) {
        /* Validate C_in structure matches: matC->nnz must equal nnzC */
        if (static_cast<int64_t>(matCInner->nnz) != desc->nnzC) {
            OP_LOGE("spgemm", "β≠0 + C_in: matC->nnz=%lu != nnzC=%ld, "
                    "C_in structure must match result structure",
                    (unsigned long)matCInner->nnz, desc->nnzC);
            return ACL_SPARSE_STATUS_INVALID_VALUE;
        }
        /* Preserve user C_in data — do not memset */
    } else {
        /* C_in empty or β=0: memset valuesC to 0 */
        if (matCInner->values != nullptr && matCInner->nnz > 0) {
            int32_t computeDtypeSize = GetComputeDtypeSize(computeType);
            int64_t valuesBytes = static_cast<int64_t>(matCInner->nnz) * computeDtypeSize;
            aclrtStream stream = nullptr;
            aclsparseGetStream(handle, &stream);
            aclError zeroRet = aclrtMemsetAsync(matCInner->values,
                                                static_cast<size_t>(valuesBytes),
                                                0, static_cast<size_t>(valuesBytes), stream);
